...
public class WorkflowEngine : IWorkflowEngine
{
    ...
    public async Task<WorkflowExecutionResult> ExecuteAsync(WorkflowDefinition workflow, WorkflowContext context)
    {
        ...
        try
        {
            ...
            var finalStatus = stepResults.All(sr => sr.Status == WorkflowStatus.Completed)
        ...
        WorkflowContext context)
    {
        ...
        foreach (var stepId in executionOrder)
        {
            ...
            try
            {
                ...
                {
                    // Создаем результат для заблокированного или пропущенного шага
                    var stepResult = CreateBlockedStepResult(stepId, dependencyCheckResult.BlockingReason, dependencyCheckResult.FailedDependencies);
                    stepResults.Add(stepResult);
                    stepResultLookup[stepId] = stepResult;
